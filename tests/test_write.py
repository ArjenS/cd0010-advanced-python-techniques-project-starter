...
class TestWriteToCSV(unittest.TestCase):
    ...
    def test_csv_data_has_header(self):
        ...
        # had a header equal to the fieldnames. (this was mentioned in several posts, such as here:
        # https://stackoverflow.com/questions/11205128/has-header-from-csv-sniffer-gives-different-results-for-files-with-same-layout)
        # Therefore I replaced the test with one that determines the first line to consist of only strings.
        # Since the other lines of the file will contain other datatypes than string a string-only line will be a header.
        buf = io.StringIO(self.value)
        try:
            reader = csv.reader(buf)
            first_line = [line for line in reader][0]
            assert all(
                isinstance(entry, str) for entry in first_line
...
class TestWriteToJSON(unittest.TestCase):
    ...
    def test_json_element_decodes_to_correct_types(self):
        ...
        if approach["neo"]["name"]:
            self.assertIs(type(approach["neo"]["name"]), str)
